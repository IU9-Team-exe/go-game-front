...
(function (b, p) {
    var g = function (u) {
        var a = new r(JSON.parse(JSON.stringify(u.getProperties()))), c;
        for (c in u.children) a.appendChild(g(u.children[c]));
        return a
    }, f = function (u, a) {
        var c;
        if (a[u] !== p) return a[u];
        for (var h in a.children) if (c = f(u, a.children[h])) return c;
        return !1
    }, l = function (a, c) {
        ...
        if (1 < c.children.length) {
            ...
            a.push(h)
                     ...
                     a) {
        for (var h = a, e, b = 1; b < c.length; b++) if (c[b].constructor == Array) for (var k = 0; k < c[b].length; k++) e = new r(c[b][k][0]), h.appendChild(e), d(c[b][k], e); else e = new r(c[b]), h.insertAfter(e), h = e
    }, k = function (c) {
    ...
    var c = function (c, a, e) {
        for (var h = 0; h < c.length; h++) if (c[h].x == a.x && c[h].y == a.y) {
            c[h][e] = a[e];
            return
        }
        c.push(a)
    }, q = function (c, a) {
        if (c) for (var e = 0; e < c.length; e++) if (c[e].x == a.x && c[e].y == a.y) {
            c.splice(e, 1);
            break
        }
    }, r = function (c, a) {
        this.parent = a || null;
        this.children = [];
        if (c) for (var e in c) this[e] = c[e]
    };
    r.prototype = {
        constructor: r, getChild: function (c) {
            c = c || 0;
            ...
            for (var a in c.children) if (c.children[a] == this) {
    ...
    var y = function (c, a) {
        for (var e = c.size, h = [], d = [], b = 0; b < e * e; b++) c.schema[b] && !a.schema[b] ? d.push({
            x: Math.floor(b / e),
            y: b % e
        }) : c.schema[b] != a.schema[b] && h.push({x: Math.floor(b / e), y: b % e, c: a.schema[b]});
        return {add: h, remove: d}
    }, e = function (c, a, e) {
        this.kifu =
            c;
        ...
        for (d in c) {
            h = !0;
            for (var b in a) if (c[d].x == a[b].x && c[d].y == a[b].y) {
                h = !1;
                break
            }
            h && e.push(c[d])
        ...
        if (a.move != p) {
            if (a.move.pass) return c.pass(a.move.c), {add: [], remove: []};
            c = c.play(a.move.x, a.move.y, a.move.c);
            if ("number" == typeof c) throw new x(c, a);
            for (var h in c) if (c[h].x == a.move.x && c[h].y == a.move.y) return {add: [], remove: c};
            return {add: [a.move], remove: c}
        }
        e || c.pushPosition();
        e = [];
        var d = [];
        if (a.setup != p) for (h in a.setup) a.setup[h].c ? (c.setStone(a.setup[h].x, a.setup[h].y, a.setup[h].c), e.push(a.setup[h])) : (c.removeStone(a.setup[h].x, a.setup[h].y), d.push(a.setup[h]));
    ...
    e.prototype = {
        constructor: e, next: function (c) {
            this.change = z.call(this, c);
            return this
        }, last: function () {
            var c;
            for (this.change = {add: [], remove: []}; c = z.call(this);) {
                var a = this.change;
...
(function (b, p) {
    ...
    f.AB = f.AW = function (a, d, k, e) {
        for (var c in k) d.addSetup({x: k[c].charCodeAt(0) - 97, y: k[c].charCodeAt(1) - 97, c: "AB" == e ? b.B : b.W})
    };
    f.AE = function (a, d, b) {
    ...
    f.CR = f.SQ = f.TR = f.SL = f.MA = function (a, d, b, e) {
        for (var c in b) d.addMarkup({x: b[c].charCodeAt(0) - 97, y: b[c].charCodeAt(1) - 97, type: e})
    };
    f.SZ = function (a, d, b) {
        a.size = parseInt(b[0])
    };
    ...
    var l = /\(|\)|(;(\s*[A-Z]+(\s*((\[\])|(\[(.|\s)*?([^\\]\]))))+)*)/g,
        d = /[A-Z]+(\s*((\[\])|(\[(.|\s)*?([^\\]\]))))+/g, k = /[A-Z]+/, a = /(\[\])|(\[(.|\s)*?([^\\]\]))/g;
    b.SGF.parse = function (h) {
        var f = [], g, e, c, q = new b.Kifu, r = null;
        h = h.match(l);
        for (var p in h) if ("(" == h[p]) f.push(r); else if (")" == h[p]) r = f.pop(); else {
            r && q.nodeCount++;
...
(function (b) {
    ...
    var g = b.loadFromUrl = function (c, a) {
        ...
        if (1 < c.node.children.length && this.config.displayVariations) for (var e = 0; e < c.node.children.length; e++) c.node.children[e].move && !c.node.children[e].move.pass && a.push({
            type: "LB",
        ...
        if (c.node.markup) {
            for (e in c.node.markup) for (var d = 0; d < a.length; d++) c.node.markup[e].x == a[d].x && c.node.markup[e].y == a[d].y && (a.splice(d, 1), d--);
        ...
        for (var e in this.kifuReader.node.children) if (this.kifuReader.node.children[e].move && this.kifuReader.node.children[e].move.x == c && this.kifuReader.node.children[e].move.y == a) {
            this.next(e);
            break
        }
    }, n = function (c) {
    ...
    n.prototype = {
        constructor: n, init: function () {
            this.kifu = null;
            this.listeners = {kifuLoaded: [l.bind(this)], update: [f.bind(this)], frozen: [], unfrozen: []};
        ...
        loadJSON: function (c, a) {
            ...
            if (this.listeners[a.type]) for (var e in this.listeners[a.type]) this.listeners[a.type][e](a)
        },
        ...
        first: function () {
            ...
            this.frozen = a;
            this.dispatchEvent({type: this.frozen ? "frozen" : "unfrozen", target: this})
        }, appendTo: function (a) {
    ...
    l.prototype.updateDimensions = function () {
        ...
        if (a != this.width || h != this.height || n != this.maxHeight) {
            ...
            a:if (b = this.config.layout, b.constructor == Array) {
                ...
                for (h = 0; h < b.length; h++) if (!b[h].conditions || !(b[h].conditions.minWidth && !(b[h].conditions.minWidth <=
            ...
            if ((this.currentLayout = b) && this.lastLayout != this.currentLayout) {
                ...
                for (var g in this.components) this.components[g]._detachFromPlayer =
                    !0;
                f.call(this, "left");
                f.call(this, "right");
                f.call(this, "top");
...
(function (b, p) {
    ...
    g.prototype = {
        constructor: g, appendTo: function (b) {
...
(function () {
    var b = function (a) {
        ...
        if (!(d == b && a.scrollHeight <= a.offsetHeight)) if (a.scrollHeight > a.offsetHeight) for (d -= 2; a.scrollHeight > a.offsetHeight && 1 < d;) a.style.fontSize = d + "px", d -= 2; else if (d < b) {
    ...
    d.prototype.setPlayerTime = function (a, b) {
        var d =
            Math.floor(b / 60), f = Math.round(b) % 60;
...
(function (b, p) {
    var g = function (a) {
        ...
        this._tmp_mark = {type: "MA", x: b, y: d};
...
(function (b, p) {
    ...
    var f = b.BasicPlayer.control = {}, l = function (a) {
    ...
    f.Widget.prototype = {
        constructor: f.Widget, init: function (a, b) {
    ...
    g.menu = [{
        constructor: f.MenuItem, args: {
            name: "switch-coo", togglable: !0, click: function (a) {
    ...
    g.widgets = [{
        constructor: f.Group,
        args: {name: "left", widgets: [{constructor: f.Button, args: {name: "menu", togglable: !0, click: h}}]}
    }, {
        constructor: f.Group, args: {
            name: "right",
            widgets: [{
                constructor: f.Button, args: {
                    name: "about", click: function (a) {
        ...
        constructor: f.Group, args: {
            name: "control", widgets: [{
                constructor: f.Button, args: {
                    name: "first", disabled: !0, init: function (b) {
                ...
                constructor: f.Button, args: {
                    name: "previous", disabled: !0, multiple: !0, init: function (b) {
                        b.addEventListener("update", l.bind(this));
                        b.addEventListener("frozen", k.bind(this));
                        b.addEventListener("unfrozen", a.bind(this))
                    }, click: function (a) {
                ...
                constructor: f.Button, args: {
                    name: "multinext", disabled: !0, multiple: !0, init: function (b) {
                        b.addEventListener("update", d.bind(this));
                        b.addEventListener("frozen", k.bind(this));
                        b.addEventListener("unfrozen", a.bind(this))
                    }, click: function (a) {
...
(function (b) {
    ...
    b.Player.Editable.prototype.set = function (f) {
        if (!this.editMode && f) this.originalReader = this.player.kifuReader, this.player.kifuReader = new b.KifuReader(this.player.kifu.clone(), this.originalReader.rememberPath, this.originalReader.allow_illegal, this.originalReader.allow_illegal), this.player.kifuReader.goTo(this.originalReader.path), this._ev_click = this._ev_click || this.play.bind(this), this._ev_move = this._ev_move ||
            ...
            for (var l = this.player.kifuReader.getPosition(), d = this.originalReader.getPosition(), k = l.size, a = [], h = [], n = 0; n < k * k; n++) l.schema[n] && !d.schema[n] ? h.push({
                x: Math.floor(n / k),
                y: n % k
            }) : l.schema[n] != d.schema[n] && a.push({x: Math.floor(n / k), y: n % k, c: d.schema[n]});
            f.change = {add: a, remove: h};
    ...
    b.BasicPlayer && b.BasicPlayer.component.Control && b.BasicPlayer.component.Control.menu.push({
        constructor: b.BasicPlayer.control.MenuItem, args: {
            name: "editmode", togglable: !0, click: function (f) {
...
(function (b) {
    ...
    p.prototype.displayScore = function () {
        for (var d = [], f = [], a = [], h = [], n = [], m = 0; m < this.position.size; m++) for (var e = 0; e < this.position.size; e++) s = this.position.get(m, e), t = this.originalPosition.get(m, e), s == g.BLACK_CANDIDATE ? d.push({
            x: m,
            y: e,
            type: "mini",
            c: b.B
        }) : s == g.WHITE_CANDIDATE ? f.push({x: m, y: e, type: "mini", c: b.W}) : s == g.NEUTRAL && a.push({
            x: m,
            y: e
    ...
    p.prototype.calculate = function () {
        ...
        for (m = !0; m;) {
            m = !1;
            for (var e = 0; e < b.size; e++) for (var c = 0; c < b.size; c++) if (f = b.get(c, e), f == g.UNKNOWN || f == g.BLACK_CANDIDATE || f == g.WHITE_CANDIDATE) {
...
(function (b, p) {
    var g = {active: !0, query: {}}, f = function (b) {
        try {
